...
function redirectToExternalHome() {
    redirect('/External/Error99');
}

function openShopCategory(category) {
    redirect('/Internal/Shop/' + category);
}

function redirectToEquipment() {
    redirect('/Internal/Dock/Equipment');
}

function redirectToItemUpgradeSystem() {
    redirect('/Internal/ItemUpgradeSystem');
}
        ...
        if (obj.nodeType == 1) {
            ...
            if (typeof(tagName) == 'object') {
                ...
                for (var i = 0; i < tagName.length; i++)
                    if (tagName[i].toLowerCase() == obj.tagName.toLowerCase()) return obj;
...
function referToShopItem(lootId, focus, inSameTab) {
    var webURL = '/Internal/Shop/byLootID/';
    webURL += lootId;
...
//
//   var external = window.open('/Internal/Payment&subaction=showDynamicItem&type='+escape(type)+'&itemID='+escape(id), "paymentglobal", "width=840,height=680,left=100,top=200");
//    external.focus();
//}
...
function PilotInvites() {
    this.handleInvites = function(type, flag) {
        ...
        switch (type) {
            case 'showLoginBonus':
                ...
                var current = val + 1;
                pilotSheet.hideInviteButton('hide');
                //invitePageDiv.style.disp lay = 'none';
    ...
    this.handleShowInviteMask = function(messLabel, lang) {
        ...
        var imgTag = jQuery(document.createElement('img'))
            .attr({
                src: '/do_img/global/text_tf.esg?l=' + lang + '&s=13&t=pilot_invite_incentive_page_send_invitation&f=eurostyle_tmed&bgcolor=green&color=white'
            })
        ...
        var imgTagNew = jQuery(document.createElement('img'))
            .attr({
                src: '/do_img/global/text_tf.esg?l=' + lang + '&s=11&t=pilot_invite_incentive_page_invite_more&f=eurostyle_tmed&bgcolor=green&color=white'
            })
            .addClass('imgButtonMailInvite')
            .appendTo(spanEleNew);
    }
...
var LoginBonus = {
    ...
    renderProgressBar: function(val) {
        ...
        var current = val + 1;
        if (val <= 5) {
            jQuery('#boniProgress').addClass('boniProgress_' + val);
            jQuery('#boni_' + val).addClass('boniHeadNextBg');
            jQuery('#boniText_' + val).addClass('boniHeadCurrent');
            val = --val;
            while (val > 0) {
                jQuery('#boniHead_' + val).addClass('boniHeadReceived');
                jQuery('#boniText_' + val).addClass('boniHeadReceived');
                jQuery('#boniIcon_' + val).addClass('boniHeadReceivedBg');
                val = --val;
            }
            while (current < 6) {
                jQuery('#boniHead_' + current).addClass('boniHeadOpen');
                jQuery('#boniText_' + current).addClass('boniHeadOpen');
                current = ++current;
            }
        } else {
            for (i = 1; i < 6; i++) {
                jQuery('#boniProgress').addClass('boniProgress_' + i);
                jQuery('#boniText_' + i).addClass('boniHeadReceived');
                jQuery('#boniIcon_' + i).addClass('boniHeadReceivedBg');
                jQuery('#boniHead_' + i).addClass('boniHeadReceived');
            }
        }
    }
}
