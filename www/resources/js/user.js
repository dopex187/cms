...
var User = {
    ...
    nanoTechFactoryShowBuff: function(key, level, slot) {
        jQuery.ajax({
            type: 'POST',
            url: '/ajax/nanotechFactory.php',
            dataType: 'json',
            data: {
                command: "nanoTechFactoryShowBuff",
                key: key,
                level: level,
                slot: slot
            },
            success: function(response) {
                if (response.result == 'OK') {
                    jQuery('#buffcontainer').html(response.code);
                    jQuery('#ntf_buff_scroll').jScrollPane({
                        showArrows: true,
                        autoReinitialise: true
                    });
                    updateConfirm(response.link);
                } else {
    ...
    nanoTechFactoryShowApis: function(droneId, slot, level) {
        jQuery.ajax({
            type: 'POST',
            url: '/ajax/nanotechFactory.php',
            dataType: 'json',
            data: {
                command: "nanoTechFactoryShowApis",
                droneId: droneId,
                level: level,
                slot: slot
            },
            success: function(response) {
                if (response.result == 'OK') {
                    console.log('result from Drones');
                    jQuery('#buffcontainer').html(response.code);
                    jQuery('#factory_progress_main').html(response.progressbar);
                    jQuery('#factory_progress_count').css({
                        'background-image': response.$droneCount
                    });
